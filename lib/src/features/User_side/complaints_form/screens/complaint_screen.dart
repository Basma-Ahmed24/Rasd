  ...
  String? output(){
    if(_outputs.isNotEmpty){
      ...
      if( _outputs[0]["label"]=="potholes"){
        dropdownValue="${AppLocalizations.of(context)?.translate("potholes")}";
    ...
    return BlocProvider(
        ...
        child: BlocConsumer<ComplaintCubit, ComplaintState>(
            ...
            builder: (context, state) {
              if(widget.id==1){
                dropdownValue="${AppLocalizations.of(context)?.translate("lighting_pole")}";
              }
              else if(widget.id==0){
                dropdownValue="${AppLocalizations.of(context)?.translate("Electricity")}";}
              else if(widget.id==2){
                dropdownValue= "${AppLocalizations.of(context)?.translate("potholes")}";}
              else{
                dropdownValue="${AppLocalizations.of(context)?.translate("lighting_pole")}";
                    ...
                    child: SingleChildScrollView(
                      child: Form(key: formkey,
                        child: Column(children: [
                          ...
                          Text("${AppLocalizations.of(context)?.translate("addnewcomplaint")}",
                          ...
                          Container(
                            ...
                            items: ["${AppLocalizations.of(context)?.translate("lighting_pole")}",
                            ...
                            "${AppLocalizations.of(context)?.translate("Infringement on street")}"].map<DropdownMenuItem<String>>(
                                  (String value) {
                          ...
                          Container(
                              ...
                              decoration: BoxDecoration(
                                  ...
                                  borderRadius: BorderRadius.all(
                                      Radius.circular(10))
                              ), child: complainsTextFormField2("${AppLocalizations.of(context)?.translate("address")}", Icons.location_on, Addresscontroller)),
                          SizedBox(height: MediaQuery
                              .of(context)
                              .size
                              .height / 30,),
  ...
  Future<void> _onUploadPressed(BuildContext context) async {
    ...
    if (formkey.currentState!.validate()) {
      final complaint = ComplaintModel(
        ...
        type: dropdownValue,
        complaintImgLink: complaintCubit.path1,
      );
...
controller: cont,
      ...
      onSaved: (newValue) {
        if (hintText!.trim() == "${AppLocalizations.of(context)?.translate("address")}") {
          address = newValue!;
        } else if (hintText.trim() == "${AppLocalizations.of(context)?.translate("describe")}") {
          descripe = newValue!;
        }
      },
      validator: (value) {
