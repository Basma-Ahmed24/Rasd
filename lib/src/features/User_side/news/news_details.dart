...
class NewsDetails extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
        ...
        ProjNewsCubit()..getNewsDataWithId(id!),
        child: BlocConsumer<ProjNewsCubit, ProjNewsState>(
            ...
            builder: (context, state) {
              ...
              if (state is projnewsLoaded) {
                ...
                converter();
                // ProjNewsCubit.get(context).func(data, currentlang.languageCode)
                // var title
                print(data);
                return Scaffold(
                  ...
                  Padding(padding: EdgeInsets.all(10),child: SingleChildScrollView(child: Column(children: [
                    ...
                    Center(child: Image.asset("assets/images/Rectangle 18 (1).png")),
                    ...
                    SizedBox(height: MediaQuery.of(context).size.height/40,),
                 Row(mainAxisAlignment: MainAxisAlignment.spaceAround,children: [
                   InkWell(onTap: (){  context.read<ProjNewsCubit>().PostComment(data[0]['proj_news_id'],null,commentController.text);
                     commentController.text="";},
                     child: Center(
                       child: Text("${AppLocalizations.of(context)?.translate("addcomment")}", style: TextStyle(
                   ...
                   Container(
                     ...
                     child: TextField(controller: commentController,
                       ...
                       decoration: InputDecoration(
                         hintText: '${AppLocalizations.of(context)?.translate("addcomment")}',hintStyle: TextStyle(fontWeight: FontWeight.w700,color: Colors.black26),
                         // set the hint text

                         contentPadding: EdgeInsets.only(
                             bottom: 10), // set content padding to move the text to the bottom
                       ),
                       cursorHeight: 20, // set the height of the cursor to move it to the bottom
                     ),),

                 ],),                    SizedBox(height: MediaQuery.of(context).size.height/40,),
                    InkWell(onTap: (){
                      navigateto(context, Comments(data[0]['proj_news_id']));
                    },
                      child: Container(width: MediaQuery.of(context).size.width/2,height: MediaQuery.of(context).size.height/20,color: green,
                        child: Center(
                          child: Text("${AppLocalizations.of(context)?.translate("showallcomments")}", style: TextStyle(
